...
function exportToExcel() {
  ...
  XLSX.writeFile(workbook, fileName);
    ...
    async function readJsonRecord(fileName) {
      const query = azureConfig.sasToken.replace(/^\?/, '');
      const url = `${getBlobBaseUrl()}/${encodeBlobPath(record.blobFileName)}?${query}`;
      const response = await fetch(url);
...
const state = {
  ...
  lastLoadedAt: null
    ...
    function getBlobBaseUrl() {
      const customerPath = getCustomerPath();
      if (!customerPath) return getContainerBaseUrl();
...
function getAzureBlobUrl(blobName) {
  ...
  return `${getBlobBaseUrl()}/${encodeBlobPath(blobName)}?${query}`;
}
...
async function listJsonBlobs() {
  ...
  if (
    ...
    !containerName ||
    !sasToken ||
    sasToken.includes('PASTA_IN')
  ) {
    throw new Error(
      'Azure-inställningar saknas. Lägg in en giltig SAS-token.'
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Kunde inte läsa från Azure (${response.status}).`
    );
  }
  ...
  const blobNodes =
    Array.from(doc.querySelectorAll('Blob'));

  return blobNodes
    .map(blob => {
      const fullName =
        blob.querySelector('Name')?.textContent || '';
      ...
      // Azure returnerar t.ex. "demo/20260910_222854.json".
      // Ta bort "demo/" eftersom getBlobBaseUrl()
      // redan innehåller kundmappen.
      const name =
        prefix && fullName.startsWith(prefix)
...
async function readJsonRecord(fileName) {
  ...
  const url =
    `${getBlobBaseUrl()}/${encodeBlobPath(fileName)}?${query}`;

  const response = await fetch(url);

  if (!response.ok) {
    throw new Error(`Kunde inte läsa ${fileName}`);
  }
